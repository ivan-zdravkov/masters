...
let ExperimentDataComponent = class ExperimentDataComponent {
    ...
    decompressImages(result, imageResult, startPlayback) {
        let zip = new jszip__WEBPACK_IMPORTED_MODULE_8__();
    ...
    updateBone(frame, jointType) {
        var threeJoint = this.getBone(this.scene.children[0].bones, jointType);
        var joint = frame.Skeleton.Joints.find(x => x.JointType === jointType);
        if (joint) {
    ...
    getBone(bones, jointType) {
        return bones.find(b => b.jointType === jointType);
    }
    connect(bones, root, leaf) {
...
ExperimentDataComponent.ctorParameters = () => [
...
class ThreeJoint extends three__WEBPACK_IMPORTED_MODULE_0__["Bone"] {
    constructor(jointType) {
        super();
        this.jointType = jointType;
    }
}
ThreeJoint.ctorParameters = () => [
    { type: undefined }
];
